...
void DAC8552a::setPins(int DIN, int SCK, int SYNC) {
	_DIN = DIN;
	_SCK = SCK;
	_SYNC = SYNC;
	pinMode(_DIN,OUTPUT);
...
void DAC8552a::initializeDAC8552a() {
	Serial.begin(9600);
	// initialize pins:
...
void DAC8552a::setChanValue(int chan, int value) {
	int header;
	//Choose of channels - define the header byte
	switch(chan) {
		case 0: //Channel A
			header = 0x10;
			break;
